...
export function AdvancedAnalyticsClient({ accounts, transactions, budgetData }) {
  ...
  const budgetPercent = budgetData?.budget
    ? Math.min((budgetData.currentExpenses / Number(budgetData.budget.amount)) * 100, 100)
    : null;
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: C.bg }}>
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 py-8 max-w-7xl">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5 mb-8">
          <div>
            ...
            <p className="text-sm text-white/35 font-poppins mt-2">
              {transactions.length.toLocaleString("en-IN")} transactions · {accounts.length} account{accounts.length !== 1 ? "s" : ""}
            </p>
          </div>
        ...
        {/* ── Active Section ── */}
        {activeSection === "overview"  && <OverviewSection kpis={kpis} filtered={filtered} accounts={accounts} dailyAvgExpense={dailyAvgExpense} recurringTx={recurringTx} budgetData={budgetData} budgetPercent={budgetPercent} timeSeriesData={timeSeriesData} range={range} />}
        {activeSection === "cashflow"  && <CashFlowSection runningBalance={runningBalance} timeSeriesData={timeSeriesData} monthlyComparison={monthlyComparison} />}
        {activeSection === "spending"  && <SpendingSection categoryData={categoryData} incomeSources={incomeSources} topExpenses={topExpenses} />}
        {activeSection === "trends"    && <TrendsSection radarData={radarData} cumulativeData={cumulativeData} accounts={accounts} />}
        {activeSection === "insights"  && <InsightsSection kpis={kpis} categoryData={categoryData} dailyAvgExpense={dailyAvgExpense} recurringTx={recurringTx} accounts={accounts} filtered={filtered} transactions={transactions} />}
        ...
        <div className="mt-10 pt-6 border-t flex items-center justify-between" style={{ borderColor:C.border }}>
          <p className="text-xs text-white/20 font-poppins">
            {range === "ALL" ? "All time" : `Last ${range}`} · {filtered.length} transactions shown
          </p>
